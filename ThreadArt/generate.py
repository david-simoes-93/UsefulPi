...
def build_through_pixels_dict(hooks, n_hooks, wheel_pixel_size, hook_pixel_size):
    """
...
def optimise_fitness(through_pixels_dict, n_hooks, image, previous_edge, darkness, lightness_penalty, list_of_lines,
    ...
    """
    # each hook counts for 2, one for each side
...
def save_plot(list_coloured_lines, list_colours, file_name, size, n_hooks, hook_pixel_size):
	"""
...
def save_plot_progress(list_coloured_lines, list_colours, file_name, size, n_hooks, hook_pixel_size, proportion_list):
    """
	...
	for prop in proportion_list:
        ...
        save_plot(list_coloured_lines_temp, list_colours, file_name_temp, size, n_hooks, hook_pixel_size)


def total_distance(lines, hooks, wheel_real_size, wheel_pixel_size, hook_pixel_size):
    """
...
def main():
    wheel_real_size = 0.560  # diameter in meters
    hook_real_size =  0.06  # diameter in meters
    wheel_pixel_size = 1500   # diameter in pixels of output image
    ...
    through_pixels_dict = build_through_pixels_dict(hooks, n_hooks, wheel_pixel_size, hook_pixel_size)

    image_m = prepare_image("web.jpg", wheel_pixel_size)
    image_w = False
    ...
    image_w = prepare_image("web_w.jpg", wheel_pixel_size, weighting=True)
    ...
    save_plot([lines], [(0,0,0)], "out", wheel_pixel_size, n_hooks, hook_pixel_size)
    save_plot_progress([lines], [(0,0,0)], "out", wheel_pixel_size, n_hooks,
                       hook_pixel_size, np.arange(0.0, 1.0, 1.0/(n_lines//100)))
    total_distance(lines, hooks, wheel_real_size, wheel_pixel_size, hook_pixel_size)
